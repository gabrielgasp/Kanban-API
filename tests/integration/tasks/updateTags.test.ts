...
describe('Tasks updateTags endpoint integration tests', () => {
  ...
  describe('When operation successfully removes a tag', () => {
    ...
    it('Should clear redis cache if exists', async () => {
      ...
      await fetchEndpoint(`${endpoint}/${taskId}/tags`, { method: 'patch', body: { operation: -1, value: 'teste' } }) // Here we make the request
  ...
  describe('When operation fails', () => {
    ...
    it('Should 500 with message when an unexpected error occurs', async () => {
      ...
      const { status, body } = await fetchEndpoint(`${endpoint}/${fakeId}/tags`, { method: 'patch',  body: { operation: 1, value: 'teste' } })
